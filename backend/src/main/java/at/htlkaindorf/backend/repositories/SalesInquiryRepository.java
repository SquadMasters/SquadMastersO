...
public interface SalesInquiryRepository extends JpaRepository<SalesInquiryEntry, SalesInquiryEntryPK> {
    ...
    List<SalesInquiryEntry> findSaleInquirySentOffers(String careername, Long playerId, String username);

    @Query("SELECT s FROM SalesInquiryEntry s WHERE s.trainerCareerPlayer.player.player_Id = ?2 AND s.trainerCareerPlayer.career.careerName = ?1 AND s.trainerCareerPlayer.club.clubName = ?3")
    List<SalesInquiryEntry> findSaleInquiryReceivedOffers(String careername, Long playerId, String clubname);
